export interface User {
  ...
  export const requestUsers = (params: Query = baseQuery): Promise<User[]> => {
    ...
    // Поправил фильтрацию массива пользователей, поменял местами метод
    // filter() и slice(), так как при изменении offset при дефолтном состоянии
    // вашей фильтрации, поиск по name и age происходит некорректно.
